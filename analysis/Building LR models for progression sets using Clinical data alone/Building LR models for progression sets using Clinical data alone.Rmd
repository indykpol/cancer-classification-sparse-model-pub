...
for (fold in 1:nfolds) {
  ...
  G1_eval <- progressed_BRCA[G1_cv_sets[[fold]]]
	...
	model <- glm(progressed ~ years_to_birth + pathologic_stage + pathology_T_stage + pathology_N_stage + pathology_M_stage + histological_type + number_of_lymph_nodes + race + ethnicity, data = clinical_data_BRCA[c(G1_train, G2_train),], family = "binomial", maxit=100,  na.action = "na.exclude")
  ...
  #predictions[c(G1_eval, G2_eval)] <- predict(model, newdata = clinical_data_BRCA[c(G1_eval, G2_eval),], type="response")
	...
	model.matrix <- model.matrix(progressed ~ as.numeric(years_to_birth) + pathologic_stage + pathology_T_stage + pathology_N_stage + pathology_M_stage + histological_type + as.numeric(number_of_lymph_nodes) + race + ethnicity, data = clinical_data_BRCA[c(G1_train, G2_train),])
	model.matrix.eval <- model.matrix(progressed ~ as.numeric(years_to_birth) + pathologic_stage + pathology_T_stage + pathology_N_stage + pathology_M_stage + histological_type + as.numeric(number_of_lymph_nodes) + race + ethnicity, data = clinical_data_BRCA[c(G1_eval, G2_eval),])
...
c(pROC::auc(response=clinical_data_BRCA$progressed, predictor=predictions), pROC::auc(response=clinical_data_BRCA$progressed, predictor=predictions_glmNet))
```
...
clinical_data_PRAD[, c("years_to_birth", "days_to_death", "days_to_last_followup", "number_of_lymph_nodes")] <- apply(clinical_data_PRAD[, c("years_to_birth", "days_to_death", "days_to_last_followup", "number_of_lymph_nodes")], 2, as.numeric)
...
for (fold in 1:nfolds) {
  ...
  G1_eval <- progressed_PRAD[G1_cv_sets[[fold]]]
	...
	model <- glm(progressed ~ years_to_birth + pathology_T_stage + pathology_N_stage + radiation_therapy + histological_type + residual_tumor + number_of_lymph_nodes, data = clinical_data_PRAD[c(G1_train, G2_train),], family = "binomial", maxit=100,  na.action = "na.exclude") # no differential race or ethnicity was observed in the cohort
  ...
  #predictions[c(G1_eval, G2_eval)] <- predict(model, newdata = clinical_data_PRAD[c(G1_eval, G2_eval),], type="response")
	...
	model.matrix <- model.matrix(progressed ~ years_to_birth + pathology_T_stage + pathology_N_stage + radiation_therapy + histological_type + residual_tumor + number_of_lymph_nodes, data = clinical_data_PRAD[c(G1_train, G2_train),])
	model.matrix.eval <- model.matrix(progressed ~ as.numeric(years_to_birth) + pathology_T_stage + pathology_N_stage + radiation_therapy + histological_type + residual_tumor + as.numeric(number_of_lymph_nodes), data = clinical_data_PRAD[c(G1_eval, G2_eval),])
...
c(pROC::auc(response=clinical_data_PRAD$progressed, predictor=predictions), pROC::auc(response=clinical_data_PRAD$progressed, predictor=predictions_glmNet))
```
...
c(pROC::auc(response=clinical_data_KIRC$progressed, predictor=predictions), pROC::auc(response=clinical_data_KIRC$progressed, predictor=predictions_glmNet))
```
...
c(pROC::auc(response=clinical_data_LUSC$progressed, predictor=predictions), pROC::auc(response=clinical_data_LUSC$progressed, predictor=predictions_glmNet))
```
